...
namespace preLude
{
	...
	public class Mind : Brain
	{
		...
		private int maxMatchesAllowed = 5;
...
//				writeToLogFile("NEW INPUT", input);
		...
		public string listenToInput(string receivedInput)
		{
			input = receivedInput;	
			string output = "";
			if(proactiveMode == true && Clock != null)
				Clock.Stop();
		...
		private void findBestMatchWithinMemory()
		{
			...
			{		
				...
				//jetzt kennen wir den höchsten Wert,
				//zeit, die entsprechend höchstenwertigen sätze rauszufischen:
				for(i = highestValue; i > 0; i--)
				{
					...
					while(re.MoveNext())
					{
						...
						if(bestMatchesList.Count >= maxMatchesAllowed)
							break;						
					}					
				}	
			}
		...
		public void startProactiveMode()
		{
			Clock = new Timer();
		    Clock.Interval = 1000;
		    Clock.Enabled = true;
		    proactiveMode = true;
		    Clock.Start();
		    Clock.Tick += new EventHandler(KillBoredom_Tick);
		}
		private void KillBoredom_Tick(object sender, System.EventArgs e)
		{
			if(sender==Clock && proactiveMode == true)
    		{
				keepTalking();
			}
		}
		...
		{
			MessageBox.Show("hi, mir langweilt!");
			return;
		}
		public void stopProactiveMode()
		{
			Clock.Stop();
			proactiveMode = false;
		}
		...
		{
			DefaultNamespace.PreLudeClient pc = new DefaultNamespace.PreLudeClient();
		...
		public int MaxMatchesAllowed {
			set {
				if(value > 0 && value < 20)
				maxMatchesAllowed = value;
			}
		}
		#endregion
	}
